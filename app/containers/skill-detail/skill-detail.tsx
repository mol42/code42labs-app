              ...
              <View style={{ flex: 1}}>
                  ...
                  <View style={{height : 10}}></View>
                  {
                    skillsState.selectedSkillSteps.map((item: SkillStepModel) => {
                      return <View key={`key-${item.id}`} style={{borderWidth : 1, borderColor : "#DDD", borderRadius : 10, padding: 10, marginBottom: 8, flexDirection : "row", justifyContent : "space-between", alignItems : "center" }}>
                      <C42Text
                        size={16}
                        fontWeight={"normal"}
                        text={item.name}
                      ></C42Text>
                      <Entypo name="chevron-with-circle-right" size={24} color={theme.buttons.primary.color} />
                  </View>
                    })
                  }
              </View>
